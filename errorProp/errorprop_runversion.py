...
mini = tf.constant([0.00000001,0.0000001,0.000001,0.00001,0.0001,0.001,0.01,0.1,1,10,100])
maxi = tf.constant([0.0000001,0.000001,0.00001,0.0001,0.001,0.01,0.1,1,10,100,180])
leng =len(arraymini)
shape1 = [2,2]
shape2 = [2,2]
ite = 100
...
sbf16 = tf.placeholder(tf.bfloat16,shape1)
sfl16 = tf.placeholder(tf.float16,shape1)
...
#calculate the sigmoid and transform to float32
sigfl16 = tf.sigmoid(sfl16)
sigfl16_fl32 = tf.cast(sigfl16,tf.float32)
#sigmoid doesn't support for bf16 yet, so first transform to float32 and then do the sigmoid calculation

sigbf16 = tf.cast(sbf16, tf.float32)
sigbf16_fl32=tf.sigmoid(sigbf16)

sigfl32 = tf.sigmoid(c1)
...
#            b = tf.random_uniform(shape2,minval=mini[i],maxval = maxi[i],dtype = tf.float32)
            ...
            cc0_sig = sess.run(sigbf16_fl32,{sigbf16:cc0_fl32})
