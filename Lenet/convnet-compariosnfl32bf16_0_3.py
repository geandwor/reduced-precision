...
convnet-comparisnfl32bf160_3, the bfloat16 calculation will be expanded to the first fc layer
"""
...
Wbf16_fl32_fc1 = weight_variable([7*7*64,1024])
bbf16_fl32_fc1 = bias_variable([1024])
#convert the first fc layer parameters into bfloa16
...
W_fc1dif = tf.reduce_mean(tf.sqrt(tf.square(tf.subtract(Wfl32_fc1,Wbf16_fl32_fc1))))
b_fc1dif = tf.reduce_mean(tf.sqrt(tf.square(tf.subtract(bfl32_fc1,bbf16_fl32_fc1))))
...
#dropout layer cannot deal with bfloat16
...
#plot the difference of entropy
import numpy as np
