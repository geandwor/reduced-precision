...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 16 13:18:45 2018

@author: uic-cs
"""
"""
Created on Sun Jul 15 23:18:00 2018
...
neural net sturcture:
    first conv layer, first max pooling layer, second conv layer, second max pooling layer, then dropout layer, last readout layers


@author: uic-cs
extension from convnet-comparisnfl32bf160_2, which has only the input X and label are defined in bf16, then cast float32, in the first conv layer, all the calculations are in bf16 except the conv2d
all the other calculations are done in float32,
convnet-comparisnfl32bf160_3, the bfloat16 calculation will be expanded to the readout layer
"""
...
#defined in float32 used for the bf16 calculations
...
conv2d_1bf16_fl32= conv2d(xbf16_fl32_image,Wbf16_conv1)+bbf16_conv1
#then convert the conv2d result to bfloat16
conv2d_1bf16 = tf.cast(conv2d_1bf16_fl32,tf.bfloat16)

hfl32_conv1 = tf.nn.relu(conv2d(xfl32_image,Wfl32_conv1)+bfl32_conv1)
hfl32_pool1 = max_pool_2X2(hfl32_conv1)

hbf16_conv1 = tf.nn.relu(conv2d_1bf16)
...
#calculate the conv2d in float 32, then cast the result to bfloat16 for relu and max_pool
conv2d2_bf16_fl32 = conv2d(hbf16_fl32_pool1,Wbf16_conv2)+bbf16_conv2
...
hbf16_pool2_flat_drop = tf.cast(hbf16_fl32_pool2_flat_drop,tf.bfloat16)

"""readout layer"""
Wfl32_fc1 = weight_variable([7*7*64,10])
bfl32_fc1 = bias_variable([10])

Wbf16_fl32_fc1= weight_variable([7*7*64,10])
bbf16_fl32_fc1 = bias_variable([10])

Wbf16_fc1 = tf.cast(Wbf16_fl32_fc1,tf.bfloat16)
bbf16_fc1 = tf.cast(bbf16_fl32_fc1,tf.bfloat16)
...
b_fc2dif = tf.reduce_mean(tf.sqrt(tf.square(tf.subtract(bfl32_fc1,bbf16_fc1))))
yfl32_conv = tf.matmul(hfl32_pool2_flat_drop,Wfl32_fc1)+bfl32_fc1

ybf16_conv = tf.matmul(hbf16_pool2_flat_drop,Wbf16_fc1)+bbf16_fc1
...
#plot difference of entropy in fl32 and bf16
fig,ax = plt.subplots()
...
plt.legend()
plt.savefig("/home/uic-cs/Desktop/summerIntern/accuracy_fl32bf16convoluted_more_iterationssetup0_3.png")
plt.show()
